...
- [page-js](packages/page-js/) An online code editor with [@godechain-js/api](https://github.com/godechain-js/api/tree/master/packages/api) access to the currently connected node.
...
In addition the following libraries are also included in the repo. These are to be moved to the [@godechain/ui](https://github.com/godechain-js/ui/) repository once it reaches a base level of stability and usability. (At this point with the framework being tested on the apps above, it makes development easier having it close)
...
1. Clone the repo locally, via `git clone https://github.com/godechain-js/apps <optional local path>`
2. Ensure that you have a recent LTS version of Node.js, for development purposes [Node >=10.13.0](https://nodejs.org/en/) is recommended.
3. Ensure that you have a recent version of Yarn, for development purposes [Yarn >=1.10.1](https://yarnpkg.com/docs/install) is required.
4. Install the dependencies by running `yarn`
5. Ready! Now you can launch the UI (assuming you have a local godechain Node running), via `yarn run start`
...
```
docker run --rm -it --name godechain-ui -e WS_URL=ws://someip:9944 -p 80:80 jacogr/godechain-js-apps:latest
```

To build a docker container containing local changes -

```
docker build -t jacogr/godechain-js-apps .
```
...
IPFS allows sharing files in a decentralized manner in a similar fashion the godechain network exchanges blocks. IPFS works best when many nodes seed the same data. Nodes can seed specific data by **pinning** them.

You can pin with the following command:

```
curl -s https://godechain.js.org/apps/ipfs/pin.json | jq -jr .IpfsHash | xargs -0 -I CID ipfs pin add --progress CID
```

Here is a script you can save as `/usr/local/bin/godechainjs-ipfs-pin.sh`:
...
```
$ /usr/local/bin/godechainjs-ipfs-pin.sh
...
SHELL=/bin/bash
HOME=/
0 * * * * /usr/local/bin/godechainjs-ipfs-pin.sh >/dev/null 2>&1
```
...
Qmd8UnRQiBobm4qb6dhiC1HoQ7SvwZrWJenoN3JPEV3iiF 480594  godechainjs.3af757ad.js
