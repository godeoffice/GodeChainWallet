...
import useExtensionCounter from '@godechain/app-settings/useCounter';
import { availableExtensions } from '@godechain/apps-config';
import { isWeb3Injected } from '@godechain/extension-dapp';
import { onlyOnWeb } from '@godechain/react-api/hoc';
import { useApi } from '@godechain/react-hooks';
import { stringUpperFirst } from '@godechain/util';
...
function BannerExtension (): React.ReactElement | null {
  ...
  const phishing = useRef<string>(t<string>('Since some extensions, such as the godechain-js extension, protects you against all community reported phishing sites, there are valid reasons to use them for additional protection, even if you are not storing accounts in it.'));
  ...
  return (
    <Banner type='warning'>
      ...
      <p>{t<string>('Accounts injected from any of these extensions will appear in this application and be available for use. The above list is updated as more extensions with external signing capability become available.')}&nbsp;
        <a
          href='https://github.com/godechain-js/extension'
          rel='noopener noreferrer'
          target='_blank'
