...
import type { ChainProperties, ChainType } from '@godechain/types/interfaces';
...
import { WsProvider } from '@godechain/api';
import { ApiPromise } from '@godechain/api/promise';
import { deriveMapCache, setDeriveCache } from '@godechain/api-derive/util';
import { ethereumChains, typesBundle, typesChain } from '@godechain/apps-config';
import { web3Accounts, web3Enable } from '@godechain/extension-dapp';
import { TokenUnit } from '@godechain/react-components/InputNumber';
import { StatusContext } from '@godechain/react-components/Status';
import ApiSigner from '@godechain/react-signer/signers/ApiSigner';
import { keyring } from '@godechain/ui-keyring';
import { settings } from '@godechain/ui-settings';
import { formatBalance, isTestChain, objectSpread } from '@godechain/util';
import { defaults as addressDefaults } from '@godechain/util-crypto/address/defaults';
  ...
  try {
    ...
    return accounts.map(({ address, meta }, whenCreated): InjectedAccountExt => ({
      ...
      meta: objectSpread({}, meta, {
        name: `${meta.name || 'unknown'} (${meta.source === 'godechain-js' ? 'extension' : meta.source})`,
        whenCreated
      })
    }));
  } catch (error) {
...
function Api ({ apiUrl, children, isElectron, store }: Props): React.ReactElement<Props> | null {
  ...
  useEffect((): void => {
    ...
    if (apiUrl.startsWith('light://')) {
      const detect = new Detector('godechain-js/apps');
    ...
    api.on('ready', (): void => {
      const injectedPromise = web3Enable('godechain-js/apps');
