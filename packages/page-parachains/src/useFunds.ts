...
import type { AccountId, BalanceOf, BlockNumber, ParaId } from '@godechain/types/interfaces';
import type { godechainRuntimeCommonCrowdloanFundInfo } from '@godechain/types/lookup';
...
import { createNamedHook, useApi, useBestNumber, useCall, useEventTrigger, useIsMountedRef, useMapKeys } from '@godechain/react-hooks';
import { BN_ZERO, u8aConcat } from '@godechain/util';
import { encodeAddress } from '@godechain/util-crypto';
...
const optFundMulti = {
  transform: ([[paraIds], optFunds]: [[ParaId[]], Option<godechainRuntimeCommonCrowdloanFundInfo>[]]): Campaign[] =>
    paraIds
      .map((paraId, i): [ParaId, godechainRuntimeCommonCrowdloanFundInfo | null] => [paraId, optFunds[i].unwrapOr(null)])
      .filter((v): v is [ParaId, godechainRuntimeCommonCrowdloanFundInfo] => !!v[1])
