...
import type { SignedBlockExtended } from '@godechain/api-derive/types';
import type { Event } from '@godechain/types/interfaces';
import type { godechainPrimitivesV1CandidateReceipt } from '@godechain/types/lookup';
import type { IEvent } from '@godechain/types/types';
import type { EventMapInfo } from './types';

import { useEffect, useState } from 'react';

import { createNamedHook, useApi, useCall } from '@godechain/react-hooks';
import { stringify } from '@godechain/util';
...
function includeEntry (map: EventMap, event: Event, blockHash: string, blockNumber: BN): void {
  try {
    const { descriptor } = (event as unknown as IEvent<[godechainPrimitivesV1CandidateReceipt]>).data[0];
