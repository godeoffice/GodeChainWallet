...
import type { SubmittableExtrinsicFunction } from '@godechain/api/types';
import type { LinkOption } from '@godechain/apps-config/endpoints/types';
import type { Option } from '@godechain/apps-config/settings/types';
import type { XcmVersionedMultiLocation } from '@godechain/types/lookup';

import React, { useMemo, useState } from 'react';

import { getTeleportWeight } from '@godechain/apps-config';
import { ChainImg, Dropdown, InputAddress, InputBalance, MarkWarning, Modal, Spinner, TxButton } from '@godechain/react-components';
import { useApi, useApiUrl, useTeleport, useWeightFee } from '@godechain/react-hooks';
import { Available } from '@godechain/react-query';
import { BN_ZERO, isFunction } from '@godechain/util';
...
const XCM_LOC = ['xcm', 'xcmPallet', 'godechainXcm'];
...
function Teleport ({ onClose }: Props): React.ReactElement<Props> | null {
  ...
  const params = useMemo(
    () => {
      // From godechain runtime 9110 (no destination weight)
