...
import type { TxCallback } from '@godechain/react-components/Status/types';
import type { Option } from '@godechain/types';
import type { BalanceOf, EthereumAddress, StatementKind } from '@godechain/types/interfaces';

import React, { useEffect, useMemo, useState } from 'react';
import styled from 'styled-components';

import { Button, Card, TxButton } from '@godechain/react-components';
...
function Attest ({ accountId, className, ethereumAddress, onSuccess, statementKind, systemChain }: Props): React.ReactElement<Props> | null {
  ...
  if (!accounts.isAccount(accountId)) {
    return (
      <Card isError>
        <div className={className}>
          {t<string>('We found a pre-claim with this godechain address. However, attesting requires signing with this account. To continue with attesting, please add this account as an owned account first.')}
          <h3>
            <FormatBalance
